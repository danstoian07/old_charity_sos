...
class archive_subarticle extends archive {
	/* ------------------------------------------------------------------------------------- */	
	public  $table_name                         = 'articles';
	public  $ida							    = 20;
	public  $permission_add_item      		    = 1;								/* */	
	public  $permission_edit_item               = 1;  								/* */		
	public  $archive_title			        	= 'Editare detalii subarticol sectie';
	public  $archive_icon_class     			= 'fa fa-info-circle';

	public  $list_archive_title			    	= 'Lista subarticole sectie';					/* Title of archive list */
	public  $list_archive_icon_class        	= 'fa fa-info-circle';						/* icon for title of archive list */
	public  $list_add_button_name               = 'Adauga subarticol';
	public  $list_edit_item_button_name         = 'Editeaza';
	
	
	public  $tabs   			    			= array(
													array('<i class="fa fa-pencil"></i> Editeaza subarticol','tab_subarticol_general'),													
	...
	public  $archive_title_top_big          	= 'Subarticole sectie';
	...
	public function ShowEdit($id) {
		...
		if ($id_decript!=0) { 
			global $oDB;
			/* --------------------- */
			$idma = el_decript_info($_REQUEST['idma']);
			$query = "SELECT name from $oDB->articles WHERE id= $idma";
			$result = $oDB->db_query($query);
			$parent_name = '';
			if ($result_line = $oDB->db_fetch_array($result)) {
				$parent_name = $result_line['name'];
			}
			$this->archive_title_top_big = 'Subarticole sectie <span class="font-bold">'.$parent_name.'</span>';
			/* --------------------- */
			$categ_name = '';
			$categ_table    = el_TableNameWithPrefix('categories');
			$query = "
			...
			$this->archive_title = 'Panou subarticol [ <span class="font-green-sharp bold uppercase">'.$categ_name.'</span> ]';
			} else { $this->archive_title = 'Adaugare subarticol'; $breadcrumb_title = $this->archive_title;}
