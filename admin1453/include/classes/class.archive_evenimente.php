...
class archive_evenimente extends archive {
	/* ------------------------------------------------------------------------------------- */	
	public  $table_name                         = 'articles';
	public  $ida							    = 30;
	public  $permission_add_item      		    = 1;								/* */	
	public  $permission_edit_item               = 1;  								/* */		
	public  $archive_title			        	= 'Editare detalii articol';
	public  $archive_icon_class     			= 'fa fa-info-circle';

	public  $list_archive_title			    	= 'Lista articole';					/* Title of archive list */
	...
	public  $tabs   			    			= array(
													array('<i class="fa fa-pencil"></i> Editeaza articol','tab_eveniment_general'),													
												);									/* Edit Panel tabs, bidimensional array Ex: (($tab_name1, $tab_class1),($tab_name1, $tab_class1) ...) */

	public  $archive_title_top_big          	= 'Articole de specialitate';
	public  $archive_title_top_small        	= '';
	public  $activate_menuitem_id           	= 13;								/* id of menuitem wich will be active when panel show */
	...
	public function ShowEdit($id) {
		...
		if ($id_decript!=0) { 
			...
			$categ_table    = el_TableNameWithPrefix('categories');
			$query = "
			SELECT $this->real_table_name.name, $this->real_table_name.slug AS product_slug FROM el_articles 
			...
			if ($result_line = $oDB->db_fetch_array($result)) {
				$categ_name = 'Articole PM';
				//$this->AddItemToActionsMore('Vezi articolul in website', $this->return_link_in_website('articole', $result_line['product_slug']), 'fa fa-globe','_blank');
			}			
