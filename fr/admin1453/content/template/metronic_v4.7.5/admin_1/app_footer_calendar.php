...
<![endif]-->
		...
		*/
		...
		<!-- calendar resources -->
		<script src='<?=__CALENDARURL__?>lib/moment.min.js'></script>
		<!--<script src='<?=__CALENDARURL__?>lib/jquery.min.js'></script>-->				
		<script src='<?=__CALENDARURL__?>fullcalendar.min.js'></script>
		<script src='<?=__CALENDARURL__?>locale-all.js'></script>
		<script>
			$(document).ready(function() {
				...
				$('#calendar').fullCalendar({
					...
					defaultDate: '<?=$this->parent->current_date_en?>',
					locale: initialLocaleCode,
					defaultView: 'agendaWeek',
					navLinks: true, // can click day/week names to navigate views
					
					firstDay: <?=(el_between($this->parent->calendar['first_day'],0,6) ? $this->parent->calendar['first_day'] : el_get_curent_day_number())?>,
					weekNumbers: true,
					...
					<?=($this->parent->calendar['show_weekend'] ? 'weekends:true,' : 'weekends:false,')?>
					
					editable: true,
					eventLimit: false, // allow "more" link when too many events
					dayClick: function(date, allDay, jsEvent, view) {
								alert('date' + date.format('DD/MMM/YYYY') + "allDay" + allDay.title + "jsEvent" + jsEvent + "view" + view);
					},					
					events: {
						url: '<?=__CALENDARURL__?>php/get-events.php?id_user=<?=$this->parent->user_id_cript?>&cc=<?=el_encript_info($this->parent->json_calendar_colors)?>&ct=<?=$calendar_type?>',
						error: function() {
							$('#script-warning').show();
						}
					},
					...
					eventDrop: function(event, delta, revertFunc) {						
						...
						if (!confirm("Mutati evenimentul la data "+event.start.format('DD-MM-YYYY')+"?")) {
							revertFunc();
						} else {
							/* aici apel ajax pentru update evenimente personale dragabile */
						}
					},
